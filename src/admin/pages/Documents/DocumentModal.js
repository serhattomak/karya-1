...
const DocumentModal = ({ document, onSave, onClose }) => {
  ...
  useEffect(() => {
    if (document) {
      const documentSlug =
        document.slug || generateSlug(document.name || "document");
      setFormData({
        name: document.name || "",
        slug: documentSlug,
  ...
  const fetchAvailableFiles = async () => {
    try {
      ...
      console.log(
        "📂 Dosya sayısı:",
        Array.isArray(files) ? files.length : "Array değil"
      );
  ...
  const handleFileSelect = (e) => {
    ...
    if (file) {
      ...
      if (file.type === "application/pdf") {
        console.log(
          "📄 PDF dosyası tespit edildi, özel kontroller yapılıyor..."
        );
        ...
        if (file.size > pdfMaxSize) {
          ...
          Swal.fire({
            ...
            text: `PDF dosyası ${Math.round(
              file.size / (1024 * 1024)
            )}MB. Maksimum 50MB olmalıdır.`,
            confirmButtonColor: "#ffc107",
          });
          return;
        }
...
%%EOF`;
  ...
  const uploadFileAndGetId = async (file, retries = 3) => {
    ...
    const maxSize =
      file.type === "application/pdf" ? 50 * 1024 * 1024 : 100 * 1024 * 1024;
    if (file.size > maxSize) {
    ...
    try {
      for (let attempt = 1; attempt <= retries; attempt++) {
        try {
          console.log(
            `📤 Dosya yükleniyor (Deneme ${attempt}/${retries}):`,
            file.name
          );

          const timeoutDuration =
            file.type === "application/pdf" ? 600000 : 300000;
          ...
          const response = await uploadFile(fileFormData, {
            ...
            onUploadProgress: (progressEvent) => {
              if (progressEvent.total) {
                ...
                console.log(
                  `📊 Upload progress (${file.name}): ${percentCompleted}%`
                );
          ...
          if (uploadedFileId) {
            ...
            throw new Error(
              "File ID alınamadı - Backend response'da ID bulunamadı. Response: " +
                JSON.stringify(response.data)
            );
          }
        } catch (error) {
          console.error(
            `❌ Dosya yükleme hatası (Deneme ${attempt}/${retries}):`,
            {
          ...
          if (attempt === retries) {
            ...
            if (file.type === "application/pdf") {
              if (
                ...
                errorMessage =
                  "PDF dosyası çok büyük. Backend PDF boyut limitini kontrol edin.";
              } else if (
                error.message.includes("415") ||
                error.response?.status === 415
              ) {
                errorMessage =
                  "PDF dosya formatı desteklenmiyor. Backend MIME type ayarlarını kontrol edin.";
              } else if (
                error.message.includes("timeout") ||
                error.code === "ECONNABORTED"
              ) {
                errorMessage =
                  "PDF yükleme zaman aşımına uğradı. Backend timeout ayarlarını kontrol edin.";
              } else if (error.message.includes("ERR_HTTP2_PROTOCOL_ERROR")) {
                errorMessage =
                  "PDF yükleme sırasında HTTP/2 protokol hatası. Backend HTTP ayarlarını kontrol edin.";
              } else if (error.message.includes("Network Error")) {
                errorMessage =
                  "PDF yükleme sırasında ağ hatası. Backend bağlantısını kontrol edin.";
              }
            } else {
              if (error.message.includes("Ağ bağlantısı hatası")) {
                ...
                error.message.includes("Sunucu bağlantı hatası") ||
                error.message.includes("ERR_HTTP2_PROTOCOL_ERROR")
              ) {
                errorMessage =
                  "Sunucu protokol hatası. Dosya çok büyük olabilir, lütfen daha küçük bir dosya deneyin.";
              } else if (error.message.includes("zaman aşımına uğradı")) {
                errorMessage =
                  "Dosya yükleme zaman aşımına uğradı. Lütfen daha küçük bir dosya deneyin.";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (selectedPreviewImage) {
        ...
        const uploadedPreviewImageId = await uploadFileAndGetId(
          selectedPreviewImage
        );
        finalFormData.previewImageFileId = uploadedPreviewImageId;

        console.log(
          "✅ Önizleme görseli yüklendi, ID:",
          uploadedPreviewImageId
        );
      }
      if (!finalFormData.slug || finalFormData.slug.trim() === "") {
        finalFormData.slug =
          generateSlug(finalFormData.name) || "document-slug";
      }
      if (finalFormData.url && finalFormData.url.trim() !== "") {
        if (!finalFormData.fileId || finalFormData.fileId.trim() === "") {
          console.log(
            "🌐 URL ile document ekleniyor, file alanları temizleniyor..."
          );
          ...
          console.log(
            "🌐 Temizlendikten sonra mimeType:",
            finalFormData.mimeType
          );
          console.log(
            "🌐 Temizlendikten sonra fileSize:",
            finalFormData.fileSize
          );
        }
      }
      ...
      if (error.response?.status === 400 && error.response?.data?.errors) {
        ...
        if (validationErrors.documentDto || validationErrors["$.fileId"]) {
          ...
          errorDetails =
            "URL ile dosya eklerken backend veri formatı sorunu oluştu. ";
          ...
          errorDetails +=
            "Lütfen tüm gerekli alanları doldurun ve tekrar deneyin.";
        } else {
      ...
      <div className="AdminDocumentModal">
        <div className="AdminModalHeader">
          ...
          <div className="AdminHeaderActions">
            ...
            <button onClick={onClose} className="AdminCloseBtn">
              ×
            </button>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="AdminModalForm">
          ...
          <div className="AdminFormGroup">
            ...
            />
            <small className="AdminFieldHint">
              Harici URL kullanıyorsanız aşağıdaki "Ana Dosya" alanını boş
              bırakabilirsiniz.
            </small>
          </div>
          ...
          <div className="AdminFormRow">
            <div className="AdminFormGroup">
              ...
              <small className="AdminFieldHint">
                Dosya yüklemek yerine yukarıdaki "Harici URL" alanını da
                kullanabilirsiniz.
              </small>
              {selectedFile && (
              ...
              {uploadingFile && uploadProgress > 0 && (
                <div className="AdminUploadProgress">
                  ...
                  <span className="AdminProgressText">
              ...
              {filePreviewUrl && (
                <div className="AdminFilePreview">
                  <img
                    ...
                    className="AdminFilePreviewImage"
                  />
                </div>
              )}
            </div>
            <div className="AdminFormGroup">
              ...
              {formData.previewImageFileId && !selectedPreviewImage && (
                <div className="AdminFileInfo">
                  <span>
                    Mevcut görsel seçili (ID: {formData.previewImageFileId})
                  </span>
                </div>
              )}
              {previewUrl && (
                <div className="AdminPreviewImageContainer">
                  <img
                    src={previewUrl}
                    alt="Önizleme"
                    className="AdminPreviewImageModal"
                  />
                </div>
              )}
            </div>
                ...
                .filter((file) => {
                  ...
                  if (typeFilter === "doc")
                    return (
                      ...
                      file.path?.match(/\.(docx?|xlsx?|pptx?)/i)
                    );
                  if (typeFilter === "other")
                    return !(
                      file.contentType?.startsWith("image/") ||
                  ...
                  >
                    ...
                    >
                      <span
                        ...
                        style={{ fontWeight: 600, color: "#333", fontSize: 14 }}
                      >
                        {file.name}
                      </span>
                      <span
